...
$(function(){
	...
	$("#tabs a").each(function(){
		...
		//当点击a元素对象时候
		$(this).click(function(){
			//获得当前点击的a元素对象的class属性
			var c = $(this).attr("class");
			//如果发现a元素的class属性含有here，则直接退出（已经点击变蓝的基础上，再次点击不进行样式切换了）
			if(c.indexOf("here") == 0){return;}
			//否则
			//获得点击的a元素ref属性tab名称（带#号的名称，放入jquery中直接变为tab页对象）
			var ref = $(this).attr("ref");
			//获得非点击a元素ref属性（here）tab名称（同上） 上一次点击
			var ref_t = tObj.attr("ref");
			//非点击a元素 添加class样式nor（超链接普通色） 
			tObj.attr("class","nor");
			//当前点击a元素的class样式为here（超链接蓝色）
			$(this).attr("class","here");
			//前一次点击的a元素ref属性tab对象隐藏
			$(ref_t).hide();
			//点击的a元素ref属性tab对象显示
			$(ref).show();
